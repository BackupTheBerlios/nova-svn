// IMPORTANT: This file is a duplicate of SystemNovaServer, with some changes,
// it will form the basis of the Local Server (hence the name).
...
namespace OpenArrow.ComponentServices
{
	public class SystemNovaServer : INovaServer
	{
		...
		public void AttachComponent(IComponent component)
		{
			...
			{
				log.Warn("[AttachComponent] Remote component with same name exists, component will be replaced");
				remoteComponents.Remove(component.Name);
			}
			manager.AddComponent(component);
			log.Info("[AttachComponent] Attached: " + component.Name);
		}
		...
		public void DetachComponent(string name)
		{
			...
			log.Info("[DetachComponent] Detached: " + name);
		}
		...
		public Dictionary<string, ComponentInfo> ResolveContract(string contractName)
		{
			...
			foreach (string comp in manager.Components)
			{
				ComponentInfo ci = manager.GetComponentInfo(comp);
				if (ci.SupportedContracts.Contains(contractName))
					dict.Add(comp, ci);
			}
		...
		/// <summary>
		/// Dispatches a control message recieved by this server component interface
		/// </summary>
		/// <param name="m">The message to dispatch</param>
		internal object Dispatch(Message m)
		{
			object r = null;
